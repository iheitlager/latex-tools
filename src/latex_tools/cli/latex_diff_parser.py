...
def group_latex_commands(tokens: List[str]) -> List[str]:
    ...
    while i < len(tokens):
        ...
        if token.startswith("\\") and len(token) > 1:
            ...
            if (
                cmd_name in FORMATTING_COMMANDS
                and i + 1 < len(tokens)
                and tokens[i + 1] == "{"
            ):
...
class LatexInlineDiffParser:
    ...
    def create_diff_document(self):
        ...
        print("\nMacros defined:")
        print("  \\odiff{text} - Red strikethrough for removed text")
...
def main():
    ...
    if len(sys.argv) != 4:
        ...
        print("\nExample:")
        print(
            "  python latex_diff_parser.py single_file.tex onefile.tex diff_output.tex"
        )
        sys.exit(1)
