...
class LaTeXProcessor:
    def __init__(
        ...
        self.labels: Dict[
            str, Dict[str, Any]
        ] = {}  # label -> {type, context, file, position}
        self.references: List[Dict[str, Any]] = []  # [{ref, type, file, position}]
        self.label_contexts: Dict[str, str] = {}  # label -> surrounding context
        self.all_label_occurrences: Dict[
            str, List[Dict[str, Any]]
        ] = {}  # Track ALL occurrences including duplicates
        self.processed_content: str = (
            ""  # Store processed content for caption extraction
        )
    ...
    def _print_summary(self) -> None:
        ...
        undefined_refs = [
            ref for ref in self.references if ref["ref"] not in self.labels
        ]
        if undefined_refs:
        ...
        if self.processed_content:
            ...
            missing_captions = [
                label for label, info in captions.items() if not info["has_caption"]
            ]
            if missing_captions:
    ...
    def _process_bibtex_only(self) -> None:
        ...
        referenced_entries = self._extract_referenced_bibtex_entries(
            bib_content, self.cited_keys
        )
    ...
    def _extract_referenced_bibtex_entries(
        self, bib_content: str, keys: List[str]
    ) -> str:
    ...
    def _extract_labels(self, content: str) -> None:
        ...
        for match in re.finditer(label_pattern, content):
            ...
            self.all_label_occurrences[label_name].append(
                {"type": label_type, "context": context, "position": position}
            )
    ...
    def _determine_label_type(self, content: str, position: int) -> str:
        ...
        for match in re.finditer(
            r"\\begin\{(figure|table|longtable|supertabular|equation|align|gather|multline|listing|lstlisting)\*?\}",
            preceding_text,
        ):
        ...
        for match in re.finditer(
            r"\\(subsubsection|subsection|section)\{", preceding_text
        ):
    ...
    def _extract_references(self, content: str) -> None:
        ...
        for match in re.finditer(ref_pattern, content):
            ...
            self.references.append(
                {"ref": ref_name, "type": ref_type, "position": match.start()}
            )
    ...
    def get_caption_report(self, content: str) -> str:
        ...
        missing = [
            label for label, info in caption_data.items() if not info["has_caption"]
        ]
        if missing:
            report_lines.append(f"\n{'=' * 60}")
            report_lines.append(
                f"⚠️  WARNING: {len(missing)} label(s) without captions:"
            )
    ...
    def _process_bibliography(self, content: str) -> str:
        ...
        if has_addbibresource and has_printbibliography:
            print(
                "Using BibLaTeX-style bibliography processing (\\addbibresource + \\printbibliography)"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Process LaTeX files by inlining includes and bibliography"
    )
    parser.add_argument(
        "input_file",
        nargs="?",
        default="main.tex",
        help="Main LaTeX file to process (default: main.tex)",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="onefile.tex",
        help="Output file (default: onefile.tex)",
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Show detailed reports"
    )
    parser.add_argument(
        "-m",
        "--mode",
    ...
    try:
        processor = LaTeXProcessor(
            args.input_file, args.output, verbose=args.verbose, mode=args.mode
        )
        processor.process()
    except Exception as e:
        print(f"Error: {e}")
