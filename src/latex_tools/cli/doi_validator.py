...
class DOIStatus(Enum):
    ...
    Exists = "Exists"
    Validated = (
        "Validated"  # DOI resolves, target accessible (401/403 - access restricted)
    )
...
class DOIValidator:
    ...
    def validate(self) -> None:
        ...
        try:
            ...
            if self.doi_results:
                print(
                    f"Partial results: {len(self.doi_results)} entries checked before interruption"
                )
    ...
    def _parse_bib_entries(self, bib_content: str) -> None:
        ...
        for match in re.finditer(entry_pattern, bib_content, re.DOTALL):
            ...
            doi_match = re.search(
                r"doi\s*=\s*\{((?:[^{}]|(?:\{[^}]*\}))*)\}", fields_str, re.IGNORECASE
            )
            if doi_match:
    ...
    def _validate_dois(self) -> None:
        ...
        for i, (key, entry) in enumerate(self.entries.items(), 1):
            ...
            if self.cache.is_valid(doi):
                ...
                if self.verbose:
                    print(
                        f"    [VERBOSE] Using cached result for {doi}: {cached_status}"
                    )
            ...
            # Check if we've reached the limit for uncached entries
                ...
                if uncached_count > self.limit:
                    ...
                    if self.verbose:
                        print(
                            f"  [{i}/{entries_with_doi}] ⏭️ {key} (limit reached, no cache)"
                        )
                    continue
                else:
    ...
    def _check_doi(self, doi: str, key: str) -> DOIStatus:
        ...
        try:
            ...
            if redirect_url != "access_restricted":
                ...
                if self.verbose:
                    print(
                        f"    [VERBOSE] Redirect target validation: {validation_status}"
                    )
    ...
    def _check_redirect(self, doi_url: str) -> Optional[str]:
        ...
        try:
            ...
            request = urllib.request.Request(
                doi_url, headers={"User-Agent": self.user_agent}
            )
            ...
            # 403/401 means access restricted but DOI exists
                ...
                if self.verbose:
                    print(
                        f"    [VERBOSE] HTTP {e.code} (access restricted but DOI likely exists)"
                    )
    ...
    def _validate_redirect_target(self, redirect_url: str) -> Optional[int]:
        ...
        try:
            request = urllib.request.Request(
                redirect_url, headers={"User-Agent": self.user_agent}
            )
            ...
            if e.code in (401, 403):
                if self.verbose:
                    print(
                        f"    [VERBOSE] Redirect target returned HTTP {e.code} (access restricted)"
                    )
                return e.code
            ...
            if self.verbose:
                print(
                    f"    [VERBOSE] Connection error validating redirect target: {type(e).__name__}"
                )
            return None
    ...
    def _print_report(self) -> None:
        ...
        exists_dois = sum(
            1 for _, status in self.doi_results.values() if status == DOIStatus.Exists
        )
        validated_dois = sum(
            1
            for _, status in self.doi_results.values()
            if status == DOIStatus.Validated
        )
        confirmed_dois = sum(
            1
            for _, status in self.doi_results.values()
            if status == DOIStatus.Confirmed
        )
        cached_dois = sum(
            1 for _, status in self.doi_results.values() if status == DOIStatus.Cached
        )
        nonexists_dois = sum(
            1
            for _, status in self.doi_results.values()
        ...
        nonexists_keys = [
            key
            for key, (doi, status) in self.doi_results.items()
            if status == DOIStatus.NonExists
        ]
...
VALIDATION STATUS MEANINGS:
    ...
    parser.add_argument(
        "--clear-cache", action="store_true", help="Clear the DOI validation cache"
    )
